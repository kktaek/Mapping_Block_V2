...
def encoder(p_input_shape):
    ...
    x6 = BatchNormalization(name = 'BatchNormalization2')(x5)
    x7 = UpSampling2D((2,2), name = 'UpSampling2D2')(x6)
    x8 = Conv2D(filters=16, kernel_size = 3, padding='same', activation='relu', name = 'Conv2D1')(x7)
    x9 = BatchNormalization(name = 'BatchNormalization2')(x8)
    x10 = UpSampling2D((2,2), name = 'UpSampling2D2')(x9)
    x11 = Conv2D(filters=1, kernel_size=3, padding='same', activation='relu', name = 'Conv2D2')(x10)
    x12 = Flatten(name = 'Flatten')(x11)
